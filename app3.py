...
def generate_2d_images(df, output_dir=image_output_dir):
    for index, row in df.iterrows():
        ...
        name = str(row.get('Ligand_Name', f"ligand_{index}")).replace(" ", "_")
...
def generate_download_links(df, output_dir=pdb_output_dir):
    ...
    for index, row in df.iterrows():
        smiles = row.get('SMILES')
        ligand_name = str(row.get('Ligand_Name', f"ligand_{index}")).replace(" ", "_")
        if pd.notna(smiles):
...
@app.route('/liquid_cancer')
def liquid_cancer():
    return render_template('liquid_cancer.html')
